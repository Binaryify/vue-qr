...
### Fixes
- Add `prepublishOnly` script to always run `build` before `npm publish`

## 5.0.2 | 2026.02.17

### Features
- Add TypeScript declarations for `vue-qr` props and `generated` event
- Export package-level types via `types` and `exports.types`

## 5.0.1 | 2026.02.17

### Fixes
- Add missing `js-binary-schema-parser` in `devDependencies` to fix build failures in clean environments

## 5.0.0 | 2026.02.04

### Breaking Changes
- Vue 3 only. For Vue 2, use v4.x (`npm install vue-qr@4`)
- `callback` prop replaced with `@generated` event

### Changes
- Migrated build tool from Webpack to Vite
- Refactored component to Vue 3 Composition API (`<script setup>`)
